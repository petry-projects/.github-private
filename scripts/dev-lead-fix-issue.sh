...
main() {
  ...
  if [ "$engine_rc" -eq 2 ]; then
    ...
    gh issue comment "$ISSUE_NUMBER" --repo "$REPO" \
      --body "<!-- dev-lead-rate-limited -->All engines are currently rate-limited. Please re-apply the \`dev-lead\` label when the rate limit clears to retry." 2>/dev/null || true
    rm -f "$prompt_file"
    exit 2
