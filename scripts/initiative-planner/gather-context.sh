...
if [[ -n $refs ]]; then
  ref_json="$(while IFS= read -r n; do
      ...
      # number — only real issues are usable prerequisite context. Skip anything
      # the issues endpoint can't return (404 for discussions / self-refs)
      # instead of failing the whole run; a genuine API/auth fault still surfaces
      # via the epics query above (which fails fast).
      gh api "repos/${REPO}/issues/${n}" --jq '{number, title, state}' 2>/dev/null \
        || echo "::notice::skipping referenced #${n} (not a fetchable issue — discussion/PR/self-ref)" >&2
    done <<<"$refs" | jq -sc '.')"
fi
