...
### Canary rollout automation (`canary-rollout.yml` / `scripts/canary-rollout.sh`)

The ring-staged, health-gated promotion (initiative #495, gate standard #548) reads
`standards/canary-rings.json` as its single source of truth for ring membership and gate knobs.

- **Dispatch surface is registry-derived (#1045).** The `agent` `workflow_dispatch` input's `options:`
  list MUST equal the agent keys in `canary-rings.json`. GitHub choice inputs are static, so the list is
  generated by `scripts/canary_agent_options.sh list` and drift-guarded by
  `scripts/canary_agent_options.sh check .github/workflows/canary-rollout.yml`
  (tests: `tests/canary_rollout.bats`). When you add/remove an agent in the registry, regenerate the
  `options:` list — otherwise dispatching that agent returns HTTP 422. `scripts/canary-rollout.sh` also
  validates the agent against the registry at runtime (`_require_agent`).
- **The 4h timer promotes (gated), it does not just evaluate.** The schedule runs `promote-all`
  (#1045): for every registry agent it advances **one ring per tick** iff that agent's #548 gate is
  `PROMOTE`. This is the "autonomous through stable, override-only" owner directive — clean candidates
  reach stable without a manual dispatch per agent per ring. The timer **never** auto-`--override`s and
  **hard-stops** on a `REGRESSION` triage (only a human `promote --override` can advance past that).
- **`gate.control.hold`** (per-agent, boolean, default false) pauses **scheduled** promotion for one
  agent while leaving manual `workflow_dispatch promote` available — the switch to hold an agent at its
  current ring without editing the workflow. `gate.control.allow_pre_existing` (already honoured) lets
  the timer advance a `BLOCKED+PRE_EXISTING` frontier (never `REGRESSION`).
- Every **real** (non-dry) promotion is journaled to `CANARY_PROMOTIONS_LOG` so the workflow records one
  GitHub Deployment per agent even when a single fleet tick advances several.
