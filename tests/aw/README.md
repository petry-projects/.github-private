...
2. **Staged smoke tests (pre-merge)** — Run a workflow against a fixture payload without posting real GitHub API side-effects. Use `--dry-run` to validate the trigger request without dispatching a run:
   ```
...
The compile step validates all `*.md` files in `.github/workflows/` against the gh-aw schema. It does not invoke Claude, but may query the GitHub API to resolve and pin action references; set `GH_TOKEN` before running if action resolution fails.

## Running staged smoke tests

```bash
# Run a single workflow against a fixture payload (dry-run: no real GitHub API side-effects)
