...
2. **Staged smoke tests (pre-merge)** — Run a workflow against a fixture payload without posting real GitHub API side-effects. Use `--staged` to dry-run:
   ```
   gh aw run <workflow-name> --staged --payload tests/aw/<workflow-name>/fixtures/<fixture>.json
   ```
...
gh aw run issue-triage --staged --payload tests/aw/issue-triage/fixtures/new-bug.json
...
- [ ] Staged smoke test passes: `gh aw run <name> --staged` exits 0
